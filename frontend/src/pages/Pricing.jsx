    ...
    <section className="py-12 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="bg-blue-50 border-l-4 border-[#004e92] rounded-r-xl p-4 mb-8 text-sm text-gray-600 max-w-3xl">
          Bảng giá chỉ mang tính tham khảo. Chi phí thực tế phụ thuộc vào tình trạng bệnh.
          <strong> Bảo hiểm y tế được áp dụng theo quy định nhà nước.</strong>
        </div>
          ...
          {categories.map((category) => (
            <div key={category.name} className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="bg-[#004e92] px-6 py-4 flex items-center gap-3">
                <span className="text-sm font-bold text-[#004e92] bg-white rounded-full w-9 h-9 flex items-center justify-center">
                  {category.icon}
                </span>
              ...
              <div className="p-6">
                {category.items.map(([name, price], index) => (
                  <div key={name} className={`flex justify-between items-center gap-4 py-3 ${index < category.items.length - 1 ? 'border-b border-gray-100' : ''}`}>
                    <span className="text-sm text-gray-700">{name}</span>
                    <span className="text-sm font-bold text-red-600 whitespace-nowrap">{price}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="text-center mt-10">
          <p className="text-gray-500 mb-4 text-sm">
            Cần tư vấn thêm về chi phí điều trị?
          </p>
          <Link to="/booking" className="inline-block bg-[#004e92] hover:bg-blue-800 text-white font-bold px-8 py-3 rounded-full transition-colors">
            Đặt lịch tư vấn
          </Link>
        </div>
      </div>
